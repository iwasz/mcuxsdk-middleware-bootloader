...
/* Reset core clock and peripherals since we can not reset peripherals using IDE tool
   as application runs in SRAMX */
static void ResetSystem(void)
{
    /*Reset main clock to FRO 12Mhz*/
    SYSCON->MAINCLKSELA = 0U;
    SYSCON->MAINCLKSELB = 0U;

    /* Reset peripherals by software, but do not reset peripherals used
     * by SPIFI controller(on board external QSPI flash) such as SPIFI, IOCON, GPIO,
     * since when debugging in external on board QSPI flash these peripherals are initialized by flash driver. */

    /*Reset peripherals in PRESETCTRL[0] */
    uint32_t original;
    original = SYSCON->PRESETCTRL[0];
    SYSCON->PRESETCTRLSET[0] = 0x087C0800;
    /* ignore SPIFI_RST bit, since ROM code set this to 1 when debugging in SRAMX
       and the flash load will set SPIFI_RST to 0 when debug in SPIFI flash */
    while (0x087C0800 != (SYSCON->PRESETCTRL[0] & ~(SYSCON_PRESETCTRL_SPIFI_RST_MASK)))
    {
    }
    SYSCON->PRESETCTRLCLR[0] = 0x087C2800;
    while (original != (SYSCON->PRESETCTRL[0]))
    {
    }

    /*Reset peripherals in PRESETCTRL[1] */
    original = SYSCON->PRESETCTRL[1];
    SYSCON->PRESETCTRLSET[1] = 0x0E4FFD85;
    while (0x0E4FFD85 != (SYSCON->PRESETCTRL[1]))
    {
    }
    SYSCON->PRESETCTRLCLR[1] = 0x0E4FFD85;
    while (original != (SYSCON->PRESETCTRL[1]))
    {
    }

    /*Reset peripherals in PRESETCTRL[2] */
    original = SYSCON->PRESETCTRL[2];
    SYSCON->PRESETCTRLSET[2] = 0x00FFF9FC;
    while (0x00FFF9FC != (SYSCON->PRESETCTRL[2]))
    {
    }
    SYSCON->PRESETCTRLCLR[2] = 0x00FFF9FC;
    while (original != (SYSCON->PRESETCTRL[2]))
    {
    }
}
