...
//    exit_vlpr();
    ...
    // Enable PORTB clock for LPSPI0, PTC11
    ...
    PCC->CLKCFG[PCC_LPUART0_INDEX] = PCC_CLKCFG_CGC(1) | PCC_CLKCFG_PCS(3);
    PCC->CLKCFG[PCC_LPI2C0_INDEX] = PCC_CLKCFG_CGC(1) | PCC_CLKCFG_PCS(3);
    PCC->CLKCFG[PCC_LPSPI0_INDEX] = PCC_CLKCFG_CGC(1) | PCC_CLKCFG_PCS(3);
...
void phantom_update(void)
{
    ...
    flash_size = (uint32_t)(kFLASH_SIZE_512KB);

    if (fsize_code == kSIM_FCFG1_PFSIZE_256KB)
    {
    ...
    ram_size = (uint32_t)(kRAM_SIZE_96KB);

    if (ram_code == kSIM_SDID_RAMSIZE_48KB)
    {
