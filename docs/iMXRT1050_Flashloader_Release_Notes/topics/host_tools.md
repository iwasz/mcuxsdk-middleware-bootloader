...
-   `MfgTool2.exe`: GUI Windows application to download and program an application image into the external flash device. cfg\_MXRT1050X.ini must be renamed to cfg.ini to use.
-   `sdphost`: command-line tool to download and execute the Flashloader application on the device. It is available for Windows, Linux, and Mac operating systems.
