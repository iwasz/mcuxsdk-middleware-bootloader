...
uint32_t spifi_nor_clk_freqOption[kSpifiSerialClk_96MHz + 1] = { 0, 24000000, 48000000, 60000000, 80000000, 96000000 };
...
status_t parse_sfdp(SPIFI_Type *base,
                    spifi_nor_config_t *config,
                    jedec_info_table_t *tbl,
                    spifi_nor_config_option_t *option);
...
status_t get_page_sector_size_from_sfdp(spifi_nor_config_t *config, jedec_info_table_t *tbl, uint8_t *sector_erase_cmd);
                    ...
                    spifi_nor_config_option_t *option)
{
    ...
    do
    {
        ...
        if (kSPIFI_CommandAllSerial != config->spifiConfig.readmemCommandFormt)
        {
            ...
            // before the stanadard is publicly released, the JESD minor revsion is still the initial version. That is
            // why we use two conditions to handle below logic.
...
status_t spifi_norflash_erase(SPIFI_Type *base,
                              spifi_nor_config_t *config,
                              uint32_t address,
                              spifi_nor_erase_type_t type)
{
    uint8_t opcode;
    ...
    if (kStatus_Success != status)
    {
        status = (kSpifiNorEraseType_Sector == type) ? kStatus_SPIFINOR_EraseSectorFail : kStatus_SPIFINOR_EraseAllFail;
    }

    return status;
}

status_t spifi_norflash_read(
    SPIFI_Type *base, spifi_nor_config_t *config, uint32_t address, uint8_t *buf, uint32_t length)
{
    spifi_command_t cmd;
