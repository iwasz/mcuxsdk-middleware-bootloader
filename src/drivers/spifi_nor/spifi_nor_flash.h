    ...
    union
    {
        struct
        {
            ...
            uint32_t quad_mode_setting : 4; //!< Quad mode enable setting requirements,
                                            //!< which is necessary for flash device which support JESD216 initial
                                            //!< version only
...
extern "C"
{
    ...
    extern status_t spifi_nor_get_config(SPIFI_Type *base,
                                         spifi_nor_config_t *config,
                                         spifi_nor_config_option_t *option);

    //!@brief Program a page data to Norflash via SPIFI.
    extern status_t spifi_norflash_program_page(SPIFI_Type *base,
                                                spifi_nor_config_t *config,
                                                uint32_t address,
                                                const uint8_t *buf);

    //!@brief Erase norflash via SPIFI.
    extern status_t spifi_norflash_erase(SPIFI_Type *base,
                                         spifi_nor_config_t *config,
                                         uint32_t address,
                                         spifi_nor_erase_type_t type);

    //!@brief Read norflash content via SPIFI.
    extern status_t spifi_norflash_read(
